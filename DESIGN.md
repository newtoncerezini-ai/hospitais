...
**The Status Is Orthogonal Rule.** Status never replaces facility classification. Green or ochre badges communicate lifecycle state, while the unit-level map reserves a dashed ochre contour for construction without changing the hospital or general-unit symbol.
...
Each facility receives a stable illustrative position inside its municipality because exact coordinates are unavailable. Hospitals use a large hospital-blue circle with a white healthcare cross; every other facility uses a compact neutral dot so hospitals lead without hiding the network. A dashed ochre outer contour identifies construction. Hover and keyboard focus reveal the facility identity, while selection emphasizes every visible symbol in the same municipality and updates the municipal detail list. Multi-select pills combine facility types, lifecycle statuses and evidence signals for advances, bed expansion and investment. The entire state keeps one quiet fill color, strong white municipal divisions and a darker external silhouette so markers remain the primary evidence.
...
- **Do** preserve the facility-type color mapping across badges, charts, and legends; on the map, use hospital blue for amplified hospital symbols and neutral points for the supporting network.
