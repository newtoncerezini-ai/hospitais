...
export function MapView({ data, units, filters, onFiltersChange, onOpenUnit }: Props) {
  ...
  useLayoutEffect(() => {
    ...
    Object.entries(data.map.fallbackCenters ?? {}).forEach(([code, center]) => {
      ...
      distributeFallback(center, municipalityUnits.length).forEach((point, index) => {
          ...
          <div ref={frameRef} className="map-frame unit-map-frame" role="region" aria-label="Mapa municipal da rede. Em telas estreitas, role horizontalmente para explorar." tabIndex={0}>
            <div ref={stageRef} className="map-stage" style={{ aspectRatio: `${viewWidth} / ${viewHeight}`, minWidth: `${MAP_MIN_WIDTH}px` }}>
              <svg className="pe-network-map" viewBox={data.map.viewBox} role="img" aria-label="Mapa de Pernambuco com uma marca por unidade de saúde">
                <g className={`map-inset ${groupByCode.has("2605459") ? "has-units is-interactive" : ""} ${highlightedCode === "2605459" ? "is-active" : ""}`.trim()} aria-label="Quadro de Fernando de Noronha" onClick={() => groupByCode.has("2605459") && setActiveCode("2605459")} onPointerEnter={() => groupByCode.has("2605459") && setPreviewCode("2605459")} onPointerLeave={() => setPreviewCode((current) => current === "2605459" ? null : current)}><rect x="875" y="10" width="118" height="76" rx="6" /><path d="M921 55c7-8 20-9 29-3-6 8-17 13-29 3Z" /><text x="884" y="28">Fernando de Noronha</text></g>
